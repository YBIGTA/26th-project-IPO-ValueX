...
def load_regression_data():
    ...
    regression_file_path = os.path.join(base_path, "df_regression_vol_sector.csv")
...
def prepare_regression_input(df):
    ...
    drop_columns = ["기업명", "year_key", "month_key", "상장일", "산업군","섹터"]
    df = df.drop(columns=drop_columns)
    return df

def save_regression_input(df):
    output_file = "regression_input_vol_sector.csv"
