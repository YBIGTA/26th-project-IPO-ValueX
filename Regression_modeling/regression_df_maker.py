...
def load_data():
    ...
    file_path = os.path.join(base_path, "X_stat_new.csv")
    ...
    daily_changerate_path = os.path.join(base_path, "daily_new_change_rate.csv")
    monthly_changerate_path = os.path.join(base_path, "monthly_new_change_rate.csv")
    sector_info_path = os.path.join(base_path, "etc", "섹터별분류.csv")

    df = pd.read_csv(file_path)
    df_monthly_volatility = pd.read_csv(monthly_volatility_path)
    df_daily_volatility = pd.read_csv(daily_volatility_path, encoding="utf-8-sig")  # 인코딩 명시
    df_sentiment = pd.read_csv(sentiment_path)
    df_monthly_change = pd.read_csv(monthly_changerate_path)
    df_daily_change = pd.read_csv(daily_changerate_path)
    ...
    # 컬럼 체크
    print("✅ df_daily_volatility columns:", df_daily_volatility.columns.tolist())

    return df, df_monthly_volatility, df_daily_volatility, df_return_ratio, df_sentiment, df_sector, df_daily_change, df_monthly_change
...
def preprocess_dates(df, df_daily_volatility):
    ...
    if "date" in df_daily_volatility.columns:
...
def merge_changerate_data(df, df_monthly_change, df_daily_change):
    # 🔹 상장일을 강제로 datetime64[ns] 형식으로 변환
    df["상장일"] = pd.to_datetime(df["상장일"], errors="coerce")

    # 🔹 df_monthly_change의 month_key 변환
    df_monthly_change["month_key"] = pd.to_datetime(df_monthly_change["month_key"], format="%Y-%m", errors="coerce").dt.strftime('%b_%y')
    df["month_key"] = df["상장일"].dt.strftime('%b_%y')

    # 🔹 월별 변동성 데이터 병합
    df = df.merge(df_monthly_change, on="month_key", how="left")

    # 🔹 df_daily_change의 date 변환
    if "date" in df_daily_change.columns:
        df_daily_change["date"] = pd.to_datetime(df_daily_change["date"], errors="coerce")

        # 🔹 데이터 타입 확인 (디버깅)
        print(f"상장일 타입: {df['상장일'].dtype}, df_daily_change['date'] 타입: {df_daily_change['date'].dtype}")

        # 🔹 병합 수행
        df = df.merge(df_daily_change[["date", "KOSPI_new1001"]], left_on="상장일", right_on="date", how="left")
...
def save_data(df):
    df.to_csv("new_df_regression_change_sector.csv", index=False)
    print("DLR 및 PPI 계산 완료. 데이터가 병합되었으며, 'df_regression_change.csv'로 저장됨.")

def main():
    df, df_monthly_volatility, df_daily_volatility, df_return_ratio, df_sentiment, df_sector, df_daily_change, df_monthly_change = load_data()
     # 상장일을 날짜 형식으로 변환 (미리 적용)
    df["상장일"] = pd.to_datetime(df["상장일"], errors="coerce")
    df, df_daily_volatility = preprocess_dates(df, df_daily_volatility)
    df = calculate_dlr(df)
    df = calculate_ppi(df)
    df = merge_changerate_data(df, df_monthly_change, df_daily_change)
