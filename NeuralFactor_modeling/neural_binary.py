...
def preprocess_data(df,name):
    """ 데이터 전처리 함수 """
    X=df[df['_id']==name].drop(columns=['_id','종가대비등락율'])
    exclude_columns = [
...
def load_xgboost_model():
    ...
    try:
        model_path = hf_hub_download(repo_id="woojz/xgboost-classifier", filename="xgboost_model.pkl")
...
y=run_predict_neural_binary("DSC인베스트먼트")
