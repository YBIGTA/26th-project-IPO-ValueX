...
# ✅ 로그 및 결과 저장 폴더 생성
os.makedirs("log/three/best_params", exist_ok=True)
os.makedirs("feature_importance/three", exist_ok=True)
os.makedirs("metrics/three", exist_ok=True)
...
def load_and_preprocess_data(file_path, selected_blocks, target_variable, vif_threshold=20):
    ...
    df_selected[target_variable] = df_target[target_variable].apply(lambda x: 0 if x <= 0 else (1 if x <= 150 else 2)) # 기준설정
...
def train_and_evaluate_models(X_train, X_test, y_train, y_test, best_params, case_num):
    models = {
        ...
        "LogisticRegression": LogisticRegression(**best_params.get("LogisticRegression", {}), multi_class="multinomial", solver="lbfgs", max_iter=1000, random_state=42)
    }
...
def main():
    ...
    for case_num in case_nums:
        ...
        log_file_path = f"log/three/classification_results_case_{case_num}.txt"
        ...
        for model_name in ["RandomForest", "XGBoost", "LightGBM", "GradientBoosting", "LogisticRegression"]:
            ...
            # ✅ 개별 모델 최적 파라미터 저장
            best_params_path = f"log/three/best_params/optuna_best_params_case_{case_num}.json"
        ...
        importance_df.to_csv(f"feature_importance/three/classification_feature_importance_case_{case_num}.csv", index=True)
        metrics_df.to_csv(f"metrics/three/classification_metrics_case_{case_num}.csv", index=True)
