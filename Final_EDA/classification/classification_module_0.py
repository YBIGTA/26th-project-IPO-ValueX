...
# ✅ 로그 및 결과 저장 폴더 생성
os.makedirs("log/one/best_params", exist_ok=True)
os.makedirs("feature_importance/one", exist_ok=True)
os.makedirs("metrics/one", exist_ok=True)
...
def load_and_preprocess_data(file_path, selected_blocks, target_variable, vif_threshold=20):
    ...
    df_selected[target_variable] = (df_target[target_variable] > 0).astype(int) # 기준설정
...
def main():
    ...
    for case_num in case_nums:
        ...
        log_file_path = f"log/one/classification_results_case_{case_num}.txt"
        ...
        for model_name in ["RandomForest", "XGBoost", "LightGBM", "GradientBoosting", "LogisticRegression"]:
            ...
            # ✅ 개별 모델 최적 파라미터 저장
            best_params_path = f"log/one/best_params/optuna_best_params_case_{case_num}.json"
        ...
        importance_df.to_csv(f"feature_importance/one/classification_feature_importance_case_{case_num}.csv", index=True)
        metrics_df.to_csv(f"metrics/one/classification_metrics_case_{case_num}.csv", index=True)
