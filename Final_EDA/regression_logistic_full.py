...
# ✅ (9) 종속변수를 0% 초과 여부로 변환 (0.2 이상으로 변경 가능)
df_selected[target_variable] = (df_selected[target_variable] > 20).astype(int)
print("\n✅ 종속변수를 0% 초과 여부로 변환 완료!")
...
logit_model = sm.Logit(df_selected[target_variable], sm.add_constant(X))
logit_result = logit_model.fit()
p_values = logit_result.pvalues.drop("const")
...
print("\n📌 [최종 유의한 변수 선택 완료]")
print(f"🔹 제거된 VIF 기준 변수: {removed_vif_features}")
print(f"🔹 제거된 P-value 기준 변수: {removed_p_features}")
...
# ✅ 평가 지표 계산
y_pred_prob = logit_result.predict(sm.add_constant(X))
y_pred = (y_pred_prob > 0.5).astype(int)

accuracy = accuracy_score(df_selected[target_variable], y_pred)
precision = precision_score(df_selected[target_variable], y_pred, zero_division=0)
recall = recall_score(df_selected[target_variable], y_pred, zero_division=0)
f1 = f1_score(df_selected[target_variable], y_pred, zero_division=0)
roc_auc = roc_auc_score(df_selected[target_variable], y_pred_prob)  # ROC-AUC Score 추가
