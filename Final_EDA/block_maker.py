...
base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__),"..","Regression_modeling"))
# 📌 3개의 입력 파일 경로
file_paths = {
    "vol": os.path.join(base_dir,"regression_input_vol_sector.csv"),
    "change": os.path.join(base_dir,"regression_input_change_sector.csv"),
    "raw": os.path.join(base_dir,"regression_input_raw_sector.csv")
}

# 📌 블록 분류 사전 정의
block_mapping = {
    "상장정보": ["단순 기관경쟁률","의무보유확약비율", "(확정)공모가격","청약경쟁률","공모후 발행주식수","최대주주 소유주 비율","최대주주 보호예수 기간","보호예수 비율"],
    "재무정보": ["유동자산", "비유동자산", "유동부채","비유동부채","자본금","이익잉여금","기타자본항목","영업이익","당기순이익","매출액"],
    "재무비율": ["부채비율", "유동비율", "영업이익률", "당기순이익률","ROE","EPS","EV/영업이익" ],
    "재무 변화율": ["부채비율 변화율", "유동비율 변화율","영업이익률 변화율","당기순이익률 변화율","매출액 변화율","ROE 변화율","EPS 변화율","EV/영업이익 변화율","DLR","PPI (%)",],
    "거시 지표": ["기업경기실사지수(BSI)","생산자물가 상승률 (PPI)", "소비심리지수(CSI)","소비자물가 상승률 (CPI)","수출입 물량지수","외환보유액","원달러 환율","통화량 (M2)","기준금리", "국고채 금리 스프레드", "무역수지",],
    "산업군 시장지수": ["KOSPI_1001", "산업코스피"],
    "커뮤니티 종목평가": ["대표매매감정점수"],
    "산업군별 뉴스 시황" : ["호황","불황"],
    "종속변수": ["종가대비등락율"]
}

# ✅ 최종 저장할 JSON 구조 (데이터까지 포함)
